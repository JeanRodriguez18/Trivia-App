import { shuffle, getcounter, getconfetti } from "./script.js";
...
function calltriviapi(){
    ...
    $.ajax({
        type: "GET",
        url: "https://opentdb.com/api.php?amount=1&category=11&encode=base64",
        data: "data",
        dataType: "json",
    }).done(function(data){
        $.each(data.results, function(index) {
            ...
            $.each(answers, function(index){
                var append_me = '<button id="films" class="btn-answers '+answers[index].value+'"> '+ atob(answers[index].text) +' </button>'
