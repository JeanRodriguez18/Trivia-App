...
function calltriviapi(){
    ...
    setTimeout(() => {
        ...
        $.ajax({
            type: "GET",
            url: "https://opentdb.com/api.php?amount=1&category=29&encode=base64",
            data: "data",
            dataType: "json",
        }).done(function(data){
            hideLoading();
            ...
            $.each(data.results, function(index) {
                ...
                $.each(answers, function(index){
                    var append_me = '<button id="comics" class="btn-answers '+answers[index].value+'"> '+ atob(answers[index].text) +' </button>'
